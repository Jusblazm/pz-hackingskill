...
local function patchHackingBooksIfModPresent()
    if getActivatedMods():contains("SimpleSkillBooks") then
        print("[HackingSkill] General: Simple Skill Books is active, patching hacking books")
        ...
        local patchItem = function(itemName, data)
            ...
            if item then
                ...
                print("[HackingSkill] Warning: Item not found -> " .. itemName)
            end
        end
        ...
        if SkillBook then
            if not SkillBook["Hacking"] then
                print("[HackingSkill] General: Successfully converted hacking books to Simple Skill Books")
                SkillBook["Hacking"] = { perk = Perks.Hacking }
            end
        else
            print("[HackingSkill] Warning: SkillBook table not found")
        end
    else
        print("[HackingSkill] General: Simple Skill Books is not installed")
    end
end
